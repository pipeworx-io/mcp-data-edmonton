...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/edmonton_recent \
  -H 'Content-Type: application/json' \
  -d '{"dataset":"311"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/edmonton_recent`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "data-edmonton": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-data-edmonton"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-data-edmonton
```
